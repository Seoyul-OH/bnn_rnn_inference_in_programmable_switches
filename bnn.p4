...
//////////////////////////////////////////
    ...
    register<bit<128>>(1024) weights;
    bit<128> bnnInput = 0;
    bit<128> XNOROutput = 0;
    bit<128> NextLayerInput = 0;
    //bit<8> count = 0;
    bit<4> activated = 0;
    ...
    action BuildInput(){
        bnnInput = ((bit<128>)hdr.ipv4.totalLen)<<8;
        bnnInput = (bnnInput + (bit<128>)hdr.ipv4.protocol)<<32;
        bnnInput = (bnnInput + (bit<128>)hdr.ipv4.srcAddr)<<32;
        bnnInput = (bnnInput + (bit<128>)hdr.ipv4.dstAddr)<<16;
        bnnInput = (bnnInput + (bit<128>)L4src)<<16;
        bnnInput = (bnnInput + (bit<128>)L4dst)<<8;
        bnnInput = bnnInput + (bit<128>)hdr.ipv4.flags;//have to check - seoyul
    }

    action XNOR(bit<128> weight){
        XNOROutput = weight^bnnInput;
        XNOROutput = ~XNOROutput;
    }

    action BitCount(bit<128> bitInput){
	...
	x = (x & m32) + ((x >> 32) & m32);
        x = (x & m64) + ((x >> 64) & m64);
        activated = (x>64) ? (bit<4>)1 : 0;
        NextLayerInput = NextLayerInput<<1;
        NextLayerInput = NextLayerInput + (bit<128>)activated;
        //NextLayerInput=(bit<128>)x;
    }
    ...
    action LayerProcess(bit<10> offset){
        ...
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+120);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+121);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+122);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+123);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+124);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+125);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+126);
        XNOR(weight);
        BitCount(XNOROutput);
        weights.read( weight, (bit<32>)offset+127);
        XNOR(weight);
        BitCount(XNOROutput);
    }
 ...
 //       hdr.udp.checksum = 0;
    ...
    apply {
		...
		hdr.ipv4.dstAddr=(bit<32>)115;
		    ...
		    bit<128> weight=0;
		    weights.read( weight, (bit<32>)128);
		    XNOR(weight);
		    BitCount(XNOROutput);
